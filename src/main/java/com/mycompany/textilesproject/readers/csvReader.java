    ...
    public static void main(String[] args) throws Exception {
        ...
        while ((row = csvReader.readLine()) != null) {
            ...
            if (ZERO==rowNumber) {
                ...
                if (  !isRecordClean){
                    //System.out.println("discarding Unclean record " + columns[columnIndicesMap_Flipped.get( "Id")]) ;
                }
        ...
        List<String> companiesWithNotEnufData =  getCompanysWithNotEnoughData(dataItemList);
        System.out.println("total compannies "+ dataItemList.size() + " not enuf data for "+ companiesWithNotEnufData.size()) ;
        for ( String   companyWithNotEnoughData : companiesWithNotEnufData){
    ...
    private static List<String>     getCompanysWithNotEnoughData ( Map <String, List<Map <String, String>>> dataItemList ){
        ...
        for ( Map.Entry  <String, List<Map <String, String>>>  entry :  dataItemList.entrySet() ){
            if (entry.getValue().size()<MINIMUM_RECORDS_PER_COMPANY){
                ...
                //System.out.println( "company " +entry.getKey()+"data items are only " + entry.getValue().size());
            }
        }
